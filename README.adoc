...
These expose to user functions that allow sending and receiving FIX messages.
If more that one FIX sessions are defined in `quickfixj.cfg`, these FIX session beans can either be annotated with `@FixSessionMapping` or implement `NamedBean`.
